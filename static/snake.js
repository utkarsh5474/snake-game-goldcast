...
var inp = prompt("Please enter your name and speed(between 1-2) separated by ~", "Name~1");
var flds = inp.split('~');
var name = flds[0];
var user_pref_speed = flds[1];
if (name.length === 0) {
	name = "Default";
}
if(user_pref_speed.length === 0) {
	user_pref_speed = "1";
}
...
function move_up(min, max, speed, w_min, w_max, h_min, h_max) {
	var snake = $("#0");
	var food = $("#food");
	if ( ( (snake.offset().top - food.offset().top >= -10) && (snake.offset().top - food.offset().top <= 10)) && ((snake.offset().left - food.offset().left >= -10) && (snake.offset().left - food.offset().left <= 10))) {
...
/*
	Function: game_end
	 Purpose: to end the game, display an alert and then display the score board
*/

function game_end(async, my_data) {
	$.ajax({
		url: "/over?name="+some_obj.pname+"&score="+some_obj.pscore,
		type: "GET",
		// contentType: "application/json",
		// dataType: "string",
		// async: true,
